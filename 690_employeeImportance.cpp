...
of employee 3. They have importance value 15, 10 and 5, respectively. Then employee 1
has a data structure like [1, 15, [2]], and employee 2 has [2, 10, [3]], and employee
3 has [3, 5, []]. Note that although employee 3 is also a subordinate of employee 1, the relationship is not direct.
...
public:
    int getImportance(vector<Employee*> employees, int id) {
        int ans = 0;
        getImportance(employees, id, ans);
        return ans;
    }
private:
    void getImportance(vector<Employee*>& employees, int id, int& ans) {
        int len = employees.size();
        for (int i = 0; i < len; i++) {
            if (employees[i]->id == id) {
                ans += employees[i]->importance;
                int sublen = (employees[i]->subordinates).size();
                if (sublen == 0) return ;
                for (int j = 0; j < sublen; j++) {
                    int newid = employees[i]->subordinates[j];
                    getImportance(employees, newid, ans);
                }
            }
        }
    }
