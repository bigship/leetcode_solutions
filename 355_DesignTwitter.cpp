...
public:
  ...
  vector<int> getNewsFeed(int userId) {
    ...
    for (auto& v : tweets[userId])
      q.push(v);

    auto followedUser = relationship[userId];
    for (auto it = followedUser.begin(); it != followedUser.end(); it++) {
      for (auto& v : tweets[*it])
        q.push(v);
    }
  ...
  void follow(int followerId, int followeeId) {
    ...
    if (relationship[followerId].count(followeeId) == 0)
