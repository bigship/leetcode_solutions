...
public:
  vector<int> killProcess(vector<int>& pid, vector<int>& ppid, int kill) {
    ...
    appendProcess(graph, ans, kill);
    return ans;
  }
private:
  void appendProcess(unordered_map<int, vector<int>>& graph, vector<int>& ans, int kill) {
    ans.push_back(kill);
    for (int i = 0; i < graph[kill].size(); i++) {
      appendProcess(graph, ans, graph[kill][i]);
    }
  }
};
