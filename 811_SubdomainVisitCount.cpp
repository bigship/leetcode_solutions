...
public:
  vector<string> subdomainVisits(vector<string>& cpdomains) {
    ...
    for (auto& domain : cpdomains) {
      extractDomainWithCnt(domain, cntmap);
    }
    for (auto it = cntmap.begin(); it != cntmap.end(); it++) {
      string str = std::to_string(it->second) + ' ' + it->first;
      res.push_back(str);
    }
    return res;
  }
private:
  void extractDomainWithCnt(string& str, unordered_map<string, int>& cntmap) {
    int idx = 0;
    ...
    int cnt = std::stoi(str.substr(0, idx));
    string domain = str.substr(idx + 1);
    cntmap[domain] += cnt;

    string sufix = "";
    while (domain.rfind('.') != string::npos) {
      int idx = domain.rfind('.');
      string c = domain.substr(idx + 1) + sufix;
      cntmap[c] += cnt;
      sufix = "." + domain.substr(idx + 1);
