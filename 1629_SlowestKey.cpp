...
public:
  char slowestKey(vector<int>& releaseTimes, string keysPressed) {
    std::map<char, int> keyDuration;
    const int n = releaseTimes.size();
    keyDuration[keysPressed[0]] = releaseTimes[0];

    int longestDuration = releaseTimes[0];
    for (int i = 1; i < n; i++) {
      int cur = releaseTimes[i] - releaseTimes[i - 1];
      longestDuration = std::max(longestDuration, cur);
      if (keyDuration.find(keysPressed[i]) != keyDuration.end()) {
        keyDuration[keysPressed[i]] = std::max(cur, keyDuration[keysPressed[i]]);
      } else {
        keyDuration[keysPressed[i]] = cur;
      }
    }

    char ans = 'a';
    for (auto it = keyDuration.rbegin(); it != keyDuration.rend(); it++) {
      if (it->second == longestDuration) {
        ans = it->first;
        break;
      }
    }
...
public:
  char slowestKey(vector<int>& releaseTimes, string keysPressed) {
    ...
    for (int i = 1; i < releaseTimes.size(); i++) {
      ...
      if (longestDuration < curDuration) {
        longestDuration = curDuration;
        ans = curChar;
      } else if (longestDuration == curDuration) {
        if (ans < curChar)
          ans = curChar;
      }
    }
