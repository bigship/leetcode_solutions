...
public:
  int createSortedArray(vector<int>& instructions) {
    std::multiset<int> m;
    int ans = 0;
    m.insert(instructions[0]);
    for (int i = 1; i < instructions.size(); i++) {
      int value = instructions[i];
      m.insert(value);
      auto less = m.lower_bound(value);
      auto greater = m.upper_bound(value);
      int left = std::distance(m.begin(), less);
      int right = std::distance(greater, m.end());
