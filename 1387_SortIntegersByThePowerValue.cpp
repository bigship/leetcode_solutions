...
public:
  int getKth(int lo, int hi, int k) {
    vector<pair<int, int>> intPowers;
    for (int i = lo; i <= hi; i++) {
      int power = powerOfInteger(i);
      intPowers.push_back({i, power});
    }

    std::sort(intPowers.begin(), intPowers.end(),
    ...
    return intPowers[k-1].first;
  }
private:
  int powerOfInteger(int n) {
    if (n == 1) return 1;
    int steps = 0;
    while (n != 1) {
      if (n & 1 == 1)
