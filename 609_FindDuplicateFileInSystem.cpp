...
the file paths of the files that have the same content. A file path is a string that has the following format:
...
public:
  vector<vector<string>> findDuplicate(vector<string>& paths) {
    ...
    // C++分词可用stringstream 配合getline
    for (auto& path : paths) {
      stringstream ss(path);
      string root;
      string s;
      getline(ss, root, ' ');   // 先提取root
      while (getline(ss, s, ' ')) {
