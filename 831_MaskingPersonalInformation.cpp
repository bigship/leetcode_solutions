...
To mask an email, all names must be converted to lowercase and all letters between the first
...
public:
  string maskPII(string S) {
    if (S.find('@') != std::string::npos)
      return maskingEmail(S);
    return maskingPhone(S);
  }
private:
  string maskingEmail(string& S) {
    int at = S.find('@');
    string right = S.substr(at);
    std::transform(right.begin(), right.end(), right.begin(), [&](unsigned char ch) -> unsigned char {
      return std::tolower(ch);
    });
    char first = tolower(S[0]);
    char last = tolower(S[at - 1]);
    return string(1, first) + "*****" + string(1, last) + right;
  }
  ...
  string maskPhone(string& S) {
    ...
    string last;
    for (int i = digits.size() - 1; i >= digits.size() - 4; i--) {
      last = digits[i] + last;
    }
    last = "***-***-" + last;
    if (digits.size() == 10)
